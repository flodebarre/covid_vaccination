...
for(ctr in c("EN", "FR")){
  ...
  # Axes, vertical
  cexx <- 0.9
  axis(2, lwd = 0, at = seq(0, maxAge, by = 10), col.axis = par("fg"), cex = cexx)
  axis(4, lwd = 0, at = seq(0, maxAge, by = 10), col.axis = par("fg"), cex = cexx)
  mtext("age", side = 2, line = 2, las = 3)

  # Horizontal axis
  axis(1, at = datesmonths$timeSinceFirstDay, 
       labels = format(datesmonths$months, "%b\n%Y"), cex.axis = cexx, padj = 1, lwd = 0, lwd.ticks = 1, tck = -0.01, col.axis = par("fg"))
  
  # Legend  
  legend(0, 100, inset = c(0, -0), col = c(color1, color2, color3), pch = 15, legend = c("Première Dose", "Dose 2 / complète", "Dose de rappel"), horiz = TRUE, yjust = 0, xpd = TRUE, bty = "n")
  ...
  mtext(pays, side = 3, line = 2, font = 2, cex = 1.5)
  
  # Credits
  mtext(side = 1, text = paste0("@flodebarre, inspiré de @VictimOfMaths, ", Sys.Date()), col = par("fg"), line = 3)
